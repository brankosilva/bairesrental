...
const DATA_FILE = path.join(ROOT, 'data', 'ventas.json');
const ADMIN_HTML = path.join(__dirname, 'ventas.html');
...
function setCORS(res) {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Venta-Id');
}
...
const server = http.createServer(async (req, res) => {
  ...
  // ── GET / → admin UI ──────────────────────────────────────
  if (req.method === 'GET' && (pathname === '/' || pathname === '/ventas.html')) {
  ...
  // ── PUT /api/ventas ───────────────────────────────────────
  if (req.method === 'PUT' && pathname === '/api/ventas') {
    try {
  ...
  // ── POST /api/upload-venta ────────────────────────────────
  // Header X-Venta-Id: id de la propiedad. Cada llamada sube UNA foto
  // con nombre único (no pisa las anteriores) para poder tener hasta 20.
  if (req.method === 'POST' && pathname === '/api/upload-venta') {
    try {
      const rawId = req.headers['x-venta-id'] || '';
      const safeId = rawId.replace(/[^a-zA-Z0-9\-_]/g, '');
      if (!safeId) { json(res, 400, { ok: false, error: 'Falta el header X-Venta-Id' }); return; }

      const ct = (req.headers['content-type'] || 'image/jpeg').split(';')[0].trim();
      const ext = EXT_FROM_MIME[ct] || '.jpg';

      const dir = path.join(ROOT, 'images', 'ventas', safeId);
      ...
      const relativePath = `./images/ventas/${safeId}/${filename}`;
...
server.listen(PORT, '127.0.0.1', () => {
  console.log('');
  console.log('  ╔═══════════════════════════════════════╗');
  console.log('  ║   BairesRental Admin — Ventas          ║');
