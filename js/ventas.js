// ventas.js — BairesRental
// Catálogo, filtros y renderizado de cards para propiedades EN VENTA
...
// ======================================================
// CATÁLOGO (desde data/ventas.json)
// ======================================================
async function cargarCatalogo() {
  const defaults = { fotos: [], fichaUrl: "", tipo: "monoambiente", direccion: "", direccionUrl: "", amenities: [] };
  try {
    const resp = await fetch("data/ventas.json?v=" + Date.now());
    if (resp.ok) {
      const data = await resp.json();
      // Las propiedades "vendido" se mantienen en el JSON para uso interno,
    ...
    console.warn("No se pudo cargar data/ventas.json");
  }
}
...
function inicializarFiltros() {
  ...
  if (sliderPrecio) {
    sliderPrecio.addEventListener("input", () => {
      ...
      labelPrecio.textContent = `USD ${parseInt(sliderPrecio.value).toLocaleString('es-AR')}`;
      aplicarFiltros();
    });
  }
  ...
  if (btnAptoCredito) {
    btnAptoCredito.addEventListener("click", () => {
      filtrosActivos.aptoCredito = !filtrosActivos.aptoCredito;
      btnAptoCredito.classList.toggle("active", filtrosActivos.aptoCredito);
      aplicarFiltros();
    });
  }
...
function filtrarPropiedades() {
  return catalogoActual
    .filter(p => {
      ...
      if (filtrosActivos.aptoCredito && !p.aptoCredito) return false;
    ...
    .sort((a, b) => {
      ...
      if (!a.esPropio && b.esPropio) return 1;
      const ord = { disponible: 0, reservado: 1, vendido: 2 };
      ...
      grid.innerHTML = `
        <div class="br-grid-full">
          <div class="text-center py-5">
            ...
            <a href="${WA_BASE}?text=${encodeURIComponent('Hola! Estoy buscando un departamento en venta en Buenos Aires. ¿Podrían ayudarme?')}"
...
function crearCardHTML(p, idx) {
  ...
  else badgeDisp = `<span class="br-badge br-badge-nodisponible">● Vendido</span>`;
  ...
  const caracTags = [];
  if (p.superficie) caracTags.push(`<span class="br-tag-servicios">${p.superficie} m²</span>`);
  if (p.ambientes) caracTags.push(`<span class="br-tag-minimo">${p.ambientes} amb.</span>`);
  if (p.aptoCredito) caracTags.push(`<span class="br-tag-servicios">🏦 Apto crédito</span>`);

  const monedaLabel = p.moneda || 'USD';
  const precioHTML = p.precio > 0
    ? `<span class="br-precio">${monedaLabel} ${p.precio.toLocaleString('es-AR')}</span><span class="br-precio-sub">${caracTags.join("")}</span>`
    : `<span class="br-precio" style="font-size:1rem;font-weight:700;">Consultar precio</span><span class="br-precio-sub">${caracTags.join("")}</span>`;

  const waLink = p.fichaUrl || "";
  const brBase = window.location.origin + window.location.pathname.replace(/[^/]*$/, "");
  const brLink = brBase + DETAIL_PAGE + "?id=" + p.id;
  const waMsgCompleto = (waLink ? `${p.whatsappMsg}\n\nFicha: ${waLink}` : p.whatsappMsg) + `\n\nLink BairesRental: ${brLink}\n\nCod: ${p.id}`;
  ...
  const primeraFoto = Array.isArray(p.fotos) && p.fotos.length ? p.fotos[0] : "";
  const imagenHTML = primeraFoto
    ? `<img src="${primeraFoto}" alt="${p.titulo}" loading="lazy" />`
    : `<div class="br-prop-img-placeholder">📸</div>`;
  const cantFotos = Array.isArray(p.fotos) ? p.fotos.length : 0;
  const fotosBadge = cantFotos > 1
    ? `<span class="br-badge" style="background:rgba(0,0,0,.6);color:#fff;margin-left:auto;">📷 ${cantFotos}</span>`
    : "";
    ...
    <div class="br-prop-card">
        ...
        <div class="br-prop-body">
          ...
          <div class="br-prop-actions">
            <button class="br-btn-detalle-primary w-100 mb-2" onclick="event.stopPropagation(); verDetalle(${idx})">
...
function leerQueryParams() {
  ...
  if (params.get("aptoCredito")) {
    filtrosActivos.aptoCredito = true;
    document.getElementById("filtro-apto-credito")?.classList.add("active");
  }
  renderizarCatalogo();
}
