...
// Cuerpo de la ficha de una venta, incluida su galería con lightbox.
// Extraído de app/pages/ventas/[id].vue por el mismo motivo que
// RentalDetailBody — ver el comentario de ese archivo.
...
function next() {
  ...
  lightboxIndex.value = (lightboxIndex.value + 1) % props.sale.fotos.length
}
function prev() {
  if (lightboxIndex.value === null) return
  lightboxIndex.value = (lightboxIndex.value - 1 + props.sale.fotos.length) % props.sale.fotos.length
}
...
const disponibilidadClass = computed(() => {
  if (props.sale.disponibilidad === 'disponible') return 'disponible'
  if (props.sale.disponibilidad === 'reservado') return 'reservado'
  return 'vendido'
})
...
const thumbs = computed(() => props.sale.fotos?.slice(1, 5) ?? [])
const remainderDesktop = computed(() => (props.sale.fotos?.length ?? 0) - 1 - 4)
const remainderMobile = computed(() => (props.sale.fotos?.length ?? 0) - 1 - 3)
...
function onShare() {
  ...
  share(props.hideBrandBadge ? props.sale.titulo : `${props.sale.titulo} — BairesRental`)
}
</script>
    ...
    <main class="br-detail">
      <div id="depto-hero" :class="{ 'depto-hero--noimg': !sale.fotos?.length }">
        ...
        <div id="depto-hero-top">
          <NuxtLink v-if="backTo" :to="backTo" class="depto-back-btn">{{ backLabel || t('detail.backToSales') }}</NuxtLink>
          <span v-else></span>
          <div class="depto-hero-badges">
            <span class="badge-disp" :class="disponibilidadClass">● {{ t(`disponibilidad.${sale.disponibilidad}`) }}</span>
            <span v-if="sale.esPropio && !hideBrandBadge" class="badge-baires">★ BairesRental</span>
          </div>
        </div>
      ...
      <div class="depto-container">
        ...
        <div class="depto-actions">
          <div class="depto-actions-row">
            <a v-if="!hideWhatsapp" :href="waHref" target="_blank" rel="noopener" class="btn-secondary-depto btn-wa-secondary" style="flex: 2">
        ...
        <NuxtLink v-if="backTo" :to="backTo" class="depto-back-bottom">{{ backLabel || t('detail.backToSales') }}</NuxtLink>
      </div>
