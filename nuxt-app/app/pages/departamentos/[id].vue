...
const disponibilidadClass = computed(() => {
  if (rental.value?.disponibilidad === 'disponible') return 'disponible'
  if (rental.value?.disponibilidad === 'reservado') return 'reservado'
  return 'no-disponible'
})

const disponibleDesdeLabel = computed(() =>
  rental.value?.disponibleDesde ? formatLongDate(rental.value.disponibleDesde, locale.value) : '',
)

const mapSrc = computed(() => mapEmbedSrc(rental.value?.direccion, rental.value?.direccionUrl))

// Prioriza el álbum de fotos; si no hay, cae a una ficha externa (Airbnb/Booking).
const fotosLink = computed(() => rental.value?.fotos || rental.value?.fichaUrl || '')
const fotosLabel = computed(() => (rental.value?.fotos ? t('detail.seeAllPhotos') : t('detail.viewFullListing')))

const waHref = computed(() =>
  whatsappUrl(
    rental.value?.whatsappMsg ||
      `Hola! Me interesa el departamento "${rental.value?.titulo}" en ${rental.value?.barrio}. ¿Podés darme más información?`,
  ),
)
...
function onShare() {
  share(rental.value ? `${rental.value.titulo} — BairesRental` : 'BairesRental')
}
</script>

<template>
  <main v-if="rental" class="br-detail">
    <div id="depto-hero" :class="{ 'depto-hero--noimg': !rental.imagen || heroImgError }">
      ...
      <div id="depto-hero-top">
        <NuxtLink :to="vendorLink(localePath('/departamentos'))" class="depto-back-btn">{{ t('detail.backToRentals') }}</NuxtLink>
        <div class="depto-hero-badges">
          <span class="badge-disp" :class="disponibilidadClass">● {{ t(`disponibilidad.${rental.disponibilidad}`) }}</span>
          <span v-if="rental.esPropio" class="badge-baires">★ BairesRental</span>
        </div>
      </div>
    ...
    <div class="depto-container">
      ...
      <div class="depto-actions">
        ...
        <div class="depto-actions-row">
          <a v-if="!isVendor" :href="waHref" target="_blank" rel="noopener" class="btn-secondary-depto btn-wa-secondary">
      ...
      <NuxtLink :to="vendorLink(localePath('/departamentos'))" class="depto-back-bottom">{{ t('detail.backToRentals') }}</NuxtLink>
    </div>
  </main>

  <main v-else class="container py-5 text-center">
    <h1 class="h4">{{ t('detail.notFoundTitle') }}</h1>
    <p><NuxtLink :to="vendorLink(localePath('/departamentos'))">{{ t('detail.backToRentals') }}</NuxtLink></p>
  </main>
</template>
